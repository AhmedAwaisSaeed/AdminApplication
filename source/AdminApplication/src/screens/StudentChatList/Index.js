...
const Index = ({navigation}) => {
  ...
  const [afterIndexChat, setAfterIndexChat] = useState(0);
  const [filterdArrayAllStudents, setFilterdArrayAllStudents] = useState([]);
  const [query, setQuery] = useState('');

  const getMoreStudents = () => {
    let lastIndex = afterIndexChat + perPageStudents;
    const nextStudents = chatList.slice(afterIndexChat, lastIndex);
  ...
  useEffect(() => {
    if (afterIndexChat < chatList.length) {
      getMoreStudents();
    }
  }, [afterIndexChat]);
  // for Search
  ...
  const filterAllStudents = () => {
    ...
    for (let i = 0, l = chatList?.length; i < l; ++i) {
      if (chatList[i].firstName.toLowerCase().includes(query.toLowerCase())) {
        filteredStudents.push(chatList[i]);
      }
    }
    ...
    <View style={styles.mainContainer}>
      <View style={styles.searchContainer}>
        <SearchComponent
          ...
          searchFromList={chatList}
          setSearchList={setFilterdArrayAllStudents}
        />
      </View>
      ...
      <View style={styles.flexOne}>
        <FlatList
          ...
          onEndReached={({distanceFromEnd}) => {
            if (afterIndexChat < chatList.length) {
              setAfterIndexChat(afterIndexChat + perPageStudents);
            }
          }}
          renderItem={renderItem}
          extraData={refreshChatList}
          keyExtractor={item => item._id.toString()}
          ListFooterComponent={
            afterIndexChat < chatList.length && !query && renderFooter
          }
        />
      </View>
    </View>
